...
- (id)init {
    self = [super init];
    if (self) {
        self.title = UM_Local(@"Help&Feedback");
...
- (void)loadViewFrame
{
    ...
    [_feedbackButton setTitle:@"帮助与反馈" forState:UIControlStateNormal];
...
//    bar.backgroundColor = UM_UIColorFromRGB(122.0, 122.0, 122.0);
    UINavigationItem *item = [[UINavigationItem alloc] initWithTitle:UM_Local(@"Help&Feedback")];
    [bar setItems:@[item]];
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillChangeFrameAction:) name:UIKeyboardWillChangeFrameNotification object:nil];
     ...
     */
//    timerMessage = [NSTimer timerWithTimeInterval:2.0 target:self selector:@selector(getDataAll) userInfo:nil repeats:YES];
//    [[NSRunLoop currentRunLoop] addTimer:timerMessage forMode:NSDefaultRunLoopMode];
}

//-(void)getDataAll
//{
//    WS(weakSelf);
//    [_feedback get:^(NSError *error) {
//        NSLog(@"----%@",_feedback.theNewReplies);
//        [weakSelf handleNewMessageWithMessage:_feedback.theNewReplies];
//    }];
//}
//
//-(void)handleNewMessageWithMessage:(NSArray *)objec
//{
//    if (!objec.count)
//    {
//        return;
//    }
//    [self.topicAndReplies addObjectsFromArray:objec];
//    [self reloadLastRow];
//}
//
//-(void)reloadLastRow
...
//    [self.mTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.topicAndReplies count]-1 inSection:0]
...
- (CGSize)getViewSize:(UIInterfaceOrientation) interfaceOrientation {
    ...
    switch (interfaceOrientation) {
        ...
        case UIInterfaceOrientationPortrait:
        case UIInterfaceOrientationPortraitUpsideDown: {
            viewWidth = kDeviceWidth;
            viewHeight= kDeviceHeight;
            navHeight = 44;
            break;
        }
        case UIInterfaceOrientationLandscapeLeft:
