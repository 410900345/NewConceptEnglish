...
- (NSValue *)myTextRange
{
    return objc_getAssociatedObject(self, kcachesIDKey);
}

- (void)setMyTextRange:(NSValue *)range
{
    objc_setAssociatedObject(self, kcachesIDKey, range, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
// change
    ...
    for(UITextSelectionRect* selectionRect in selectionRects){
        ...
        if (aRect.origin.y - tempRect.origin.y > aRect.size.height-2)
        {
            continue;
        }
...
-(UIBezierPath*)selectionPathWithGranularity:(UITextGranularity)granularity atIndex:(NSUInteger)idx{
    ...
    NSRange selectRange =   NSMakeRange(location, length);
    
    NSRange selectRangeNew = [self filteNoEnglishCharWithRange:selectRange];
    self.myTextRange = [NSValue valueWithRange:selectRangeNew];
    NSArray* rects = [self selectionRectsForRange:textRange];
    return [self _bezierPathWithTextSelectionRects:rects];
}


-(NSRange)filteNoEnglishCharWithRange:(NSRange )range
{
    NSString *newStr = [self.text substringWithRange:range];
    newStr = [newStr stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSCharacterSet *set = [NSCharacterSet characterSetWithCharactersInString:@"@／：；（）¥「」＂、[]{}#%-*+=_|.~＜＞$€^•'@#$%^&*()_+．'0123456789"];
    NSString *trimmedString = [newStr stringByTrimmingCharactersInSet:set];
    NSRange rangeNew = [self.text rangeOfString:trimmedString options:NSLiteralSearch range:range];
    return rangeNew;
}
